 ...
 *
 * 解题思路：
 *      ops中针对的是矩阵行列的操作，循环每次操作，找出最小的行操作数据和最小的列操作数，他们的乘积就是每次操作（+1）都会涉及到的，也就是最大的值的个数
 * @Date 2021-11-07 22:20
 * @Version V1.0
 **/
...
public class code598 {
    public static int maxCount(int m, int n, int[][] ops) {
        ...
        for (int i = 0; i < ops.length; i++) {
            minl=minl<ops[i][0]?minl:ops[i][0];
            minr=minr<ops[i][1]?minr:ops[i][1];
        }
        return minl*minr;
    }
